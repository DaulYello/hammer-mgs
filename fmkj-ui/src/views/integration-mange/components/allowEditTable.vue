            ...
            on: {
                'click': () => {
                    if (!currentRow.editting) {
                        ...
                        vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                        if (vm.edittingStore[index].fyMin === '') {
                            vm.$Message.error('飞羽区间最小值不能有空！')
                            return;
                        }
                        if (vm.edittingStore[index].fyMax === '') {
                            vm.$Message.error('飞羽区间最大值\t不能为空！')
                            return;
                        }
                        if (vm.edittingStore[index].weight === '') {
                            vm.$Message.error('权重不能有空！')
                            return;
                        }
                        if (vm.edittingStore[index].allotPercent === '') {
                            vm.$Message.error('分配比例不能为空！')
                            return;
                        }
                        if (vm.edittingStore[index].dilutPercent === '') {
                            vm.$Message.error('稀释比例不能有空！')
                            return;
                        }
                        if (vm.edittingStore[index].allotDesc === '') {
                            vm.$Message.error('总分配描述不能为空！')
                            return;
                        }
                        editIntegralRule(vm.thisTableData[index]).then(data => {
                            if (data.status === 200) {
