...
<template>
    ...
    <div>
        <Table :ref="refs" @on-selection-change="handleSelectionChange" @on-select-all="handleSelectionChange"
               @on-select="handleSelectionChange"  refs="multipleTable" :columns="columnsList"
               :data="thisTableData" border disabled-hover></Table>
    </div>
            ...
            on: {
                'click': () => {
                    if (!currentRow.editting) {
                        ...
                        vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                        if (vm.edittingStore[index].promptText === '') {
