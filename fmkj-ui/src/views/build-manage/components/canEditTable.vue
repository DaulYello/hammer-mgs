        ...
        on: {
            'click': () => {
                if (!currentRow.editting) {
                    ...
                    vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                    if (vm.edittingStore[index].name === '') {
                        vm.$Message.error('门票名称不能为空！')
                        return;
                    }
                    if (vm.edittingStore[index].type === '') {
                        vm.$Message.error('门票类型不能为空！')
                        return;
                    }
                    if (vm.edittingStore[index].price === '') {
                        vm.$Message.error('门票单价不能为空！')
                        return;
                    }
                    let  p = vm.edittingStore[index].price.replace(/^\d+(\.\d+)?$/,'');
                    if (p !== '') {
                        vm.$Message.error('门票单价只能输入数值！')
                        console.log(currentRow)
                        currentRow.editting = false;
                        vm.edittingStore[index].editting = true;
                        vm.edittingStore[index].saving = false;
                        return;
                    }
                    update(vm.thisTableData[index]).then(data => {
                        if (data.status === 200) {
                            //vm.loading = false;
                            //vm.loading = false;
    ...
    return h('Button', {
        ...
        on: {
            click: () => {
                ...
                let query = {
                    build_id: vm.thisTableData[index].buildId
                };
